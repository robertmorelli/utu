...
import { UtuLanguageService, type UtuSymbol } from '../../lsp/src/core/languageService';
import { toVscodeRange } from './adapters/core';
import { formatError, type RuntimeHost } from './compilerHost';
...
export function registerTesting(
  ...
  const refreshFile = async (uri: vscode.Uri) => {
    ...
    const symbols = (await dependencies.languageService.getDocumentIndex(document)).topLevelSymbols.filter(
      isRunnableSymbol,
    );
    ...
    const ordinalByKind = new Map<'test' | 'bench', number>([
      ['test', 0],
      ['bench', 0],
    ]);
    ...
    fileItem.children.replace(
      symbols.map((symbol) => {
        const kind = symbol.kind as 'test' | 'bench';
        const ordinal = ordinalByKind.get(kind) ?? 0;
        ordinalByKind.set(kind, ordinal + 1);
        const item = controller.createTestItem(
          `${uri.toString()}#${kind}:${symbol.range.start.line}:${symbol.range.start.character}`,
          symbol.name,
          uri,
        );
      ...
      for (const [uriString, items] of itemsByFile) {
        ...
        try {
          if (kind === 'test') {
            ...
            for (const item of items) {
              const data = itemData.get(item);
              if (!data || data.kind !== 'bench') continue;
              const result = results[data.ordinal];
              ...
              appendLogs(
                ...
                item,
                result.logs,
                `${result.name}: mean ${formatMs(result.meanMs)}, min ${formatMs(result.minMs)}, max ${formatMs(result.maxMs)}, ${formatMs(result.perIterationMs)}/iter`,
              );
              run.passed(item, result.durationMs);
            }
          }
  ...
  const testProfile = controller.createRunProfile('Run Tests', vscode.TestRunProfileKind.Run, (request, token) =>
    runItems(request, token, 'test'),
  );
  testProfile.tag = TEST_TAG;

  const benchProfile = controller.createRunProfile('Run Benchmarks', vscode.TestRunProfileKind.Run, (request, token) =>
    runItems(request, token, 'bench'),
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidOpenTextDocument((document) => {
      if (document.languageId === 'utu') void refreshFile(document.uri);
    }),
    vscode.workspace.onDidChangeTextDocument((event) => {
      if (event.document.languageId === 'utu') scheduleRefresh(event.document.uri);
    }),
...
function appendLogs(run: vscode.TestRun, item: vscode.TestItem, logs: string[], summary?: string): void {
  const lines = [...logs];
...
function formatMs(value: number): string {
  ...
  if (value >= 0.001) return `${(value * 1000).toFixed(3)}us`;
  return `${(value * 1_000_000).toFixed(0)}ns`;
}

function createRefreshScheduler(refreshFile: (uri: vscode.Uri) => Promise<void>) {
