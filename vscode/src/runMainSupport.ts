...
export interface HostImportRef {
  module: string;
  name: string;
}

export function getRunMainBlockerMessage(source: string): string | undefined {
...
export function collectUnsupportedRunMainImports(source: string): HostImportRef[] {
  const unsupported: HostImportRef[] = [];
