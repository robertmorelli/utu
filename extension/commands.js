...
export function registerCommands(context, d) {
    ...
    const showGenerated = (name, kind, load) => register(name, async (document = await withUtuDocument()) => document && revealGenerated(kind, document.uri, await load(document)));
    register('utu.noop', () => { });
    register('utu.compileCurrentFile', async (document = await withUtuDocument()) => document && vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Compiling ${displayNameForDocument(document)}` }, async () => {
    ...
    register('utu.runMain', async (target, document = await withUtuDocument(target)) => {
        if (!document) return;
...
async function fail(output, run) { try { return await run(); } catch (error) { output.appendLine(`[utu] ${JSON.stringify(error)}`); output.show(true); await vscode.window.showErrorMessage('UTU command failed. Check the UTU output channel for details.'); } }

function show(output, title, logs, result) { output.appendLine(`[utu] ${title}`); for (const line of logs) output.appendLine(line); if (result !== undefined) output.appendLine(String(result)); output.show(true); }
