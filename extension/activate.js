...
export function activateUtuExtension(context, options) {
    const output = vscode.window.createOutputChannel('UTU'), generatedDocuments = new GeneratedDocumentStore(), parserService = new UtuParserService({ grammarWasmPath: options.grammarWasmPath, runtimeWasmPath: options.parserRuntimeWasmPath }), languageService = new UtuLanguageService(parserService), workspaceSymbols = createWorkspaceSymbolController(languageService, output), diagnostics = new DiagnosticsController(languageService, output, options.diagnosticsCompilerHost ?? options.compilerHost), statusBarItem = options.showCompileStatusBar === false ? undefined : createCompileStatusBarItem(), refreshMainContext = createMainContextRefresher(languageService, options.runtimeHost), workspaceWatcher = vscode.workspace.createFileSystemWatcher(UTU_GLOB);
