...
// This benchmark's documented input is exactly 256 KiB. Bounding the safe
// workspace to that workload avoids reserving an unrelated megabyte on stack.
const INPUT_CAPACITY: usize = 262_144;
// The host writes through the exported linear-memory address. Atomic cells are
// the safe Rust representation for memory that can change outside this module.
...
pub extern "C" fn run(length: i32) -> i32 {
    ...
    // A safe slice requires owned ordinary bytes. This copy is intentionally
    // part of the safe benchmark's timed path.
    let mut buffer = [0u8; INPUT_CAPACITY];
    for (target, source) in buffer[..length].iter_mut().zip(INPUT[..length].iter()) {
...
#[used]
static KEEP_INPUT_PTR: extern "C" fn() -> i32 = input_ptr;
#[used]
