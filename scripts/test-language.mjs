...
if (await runNamedCases([
    ...cases.map(([name, args, code, text, stdin]) => [name, async () => {
        const { output, exitCode } = await runCli(args, stdin);
        if (exitCode !== code || !output.includes(text))
            throw new Error(output.trim());
    }]),
    ...(!process.argv.includes('--bench-examples') ? [] : benchExampleCases.map(([name, path, labels]) => [name, async () => {
