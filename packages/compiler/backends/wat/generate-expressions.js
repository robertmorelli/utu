...
const ExpressionGeneratorMixin = class {
    ...
    genBody(stmts, out, isFnBody = false, tailHint = null) {
        for (let index = 0; index < stmts.length; index += 1) {
            ...
            const expectsValue = index === stmts.length - 1
    ...
    genNsCall(node, out, explicitArgs = null, hint = null) {
        ...
        const args = explicitArgs ?? this.args(node);
    ...
    genPromote(node, hint, out) {
        ...
        this.pushGenerated(out, (lines) => {
            ...
            lines.push("  (then");
            if (elseBlock) {
                this.pushGenerated(lines, (inner) => this.genBody(kids(elseBlock), inner, false, branchType), "    ");
            }
            lines.push("  )");
            lines.push("  (else");
            this.pushGenerated(lines, (inner) => {
                inner.push(`local.get $${tempName}`);
                inner.push("ref.as_non_null");
                inner.push(`local.set $${ident}`);
            }, "    ");
            this.pushGenerated(lines, (inner) => this.genBody(kids(thenBlock), inner, false, branchType), "    ");
            lines.push("  )");
            lines.push(")");
        });
    }
    ...
    genTypeMatch(node, arms, hint, out) {
        ...
        const labels = typedArms.map((_, index) => `$__alt_${node.id}_${index}`);
        const tempName = this.altSubjectTempName(node);

        out.push(`(block ${exitLabel}${resultType ? ` (result ${resultType})` : ""}`);
        for (let index = typedArms.length - 1; index >= 0; index -= 1) {
            out.push(`  (block ${labels[index]} (result (ref $${typedArms[index].guard}))`);
        }
        this.genExpr(subject, null, out);
        out.push(`local.set $${tempName}`);
        for (let index = 0; index < typedArms.length; index += 1) {
            out.push(`local.get $${tempName}`);
            out.push(`br_on_cast ${labels[index]} (ref $${subjectType}) (ref $${typedArms[index].guard})`);
        }
        ...
        for (let index = 0; index < typedArms.length; index += 1) {
            out.push(")");
            if (typedArms[index].pattern !== "_") out.push(`local.set $${typedArms[index].pattern}`);
            else out.push("drop");
            this.genBranchExpr(typedArms[index].expr, resultType, out);
            out.push(`br ${exitLabel}`);
        }
    ...
    genLet(node, out) {
        ...
        for (let index = targets.length - 1; index >= 0; index -= 1) {
            out.push(`local.set $${targets[index].name}`);
        }
    }
    ...
    genArrayInit(node, out) {
        const elemType = parseType(kids(node)[0]);
        const method = kids(node)[1].text;
        const args = this.args(node);
