...
const ExpressionEmitterMixin = class {
    emitExpr(node, ctx) {
        switch (node.type) {
            case 'literal':
                return node.text;
...
for (const name of Object.getOwnPropertyNames(ExpressionEmitterMixin.prototype)) {
    if (name !== 'constructor') ModuleExpander.prototype[name] = ExpressionEmitterMixin.prototype[name];
}
